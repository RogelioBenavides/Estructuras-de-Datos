...
bool BST<T>::insert(T valor)
{
    ...
    while (true)
    {
        // Si el auxiliar llega a ser nullptr se crea un nodo del lado izquierdo o derecho dependiendo del caso
        if (aux == nullptr)
        {
            // Si el valor a insertar es mayor a el auxiliar se crea un nodo a la derecha
            if(valor > prev -> data){
                prev -> right = new NodeT<T>(valor);
                cout << "Se ingres¢ el valor " << prev -> right -> data << " al  rbol" << " del lado derecho" << endl;
            // Si el valor a insertar es menor a el auxiliar se crea un nodo a la izquierda
            } else{
                prev -> left = new NodeT<T>(valor);
                cout << "Se ingres¢ el valor " << prev -> left -> data << " al  rbol" << " del lado izquierdo" << endl;
            }
            // Cuando ingresa el dato se sale del m‚todo
            return true;
        }
        // Si el valor del dato ingresado es igual al del dato del auxiliar, no se puede a¤adir al  rbol
        else if (aux->data == valor)
        {
            cout << "El valor que se intent¢ ingresar ya se encuentra en el  rbol" << endl;
            return false;
        }
        // Si el valor del dato a buscar es menor al dato del auxiliar se recorre a la izquierda
        else if (valor < aux->data)
        {
            prev = aux;
            aux = aux->left;
        }
        // Si el valor del dato a buscar es mayor al dato del auxiliar se recorre a la derecha
        else
        {
            prev = aux;
            aux = aux->right;
